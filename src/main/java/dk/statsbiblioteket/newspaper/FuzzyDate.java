...
public final class FuzzyDate implements Comparable<FuzzyDate> {
    ...
    public FuzzyDate(String dateString) throws ParseException {
    ...
    /**
     * Tests if this FuzzyDate is after the specified FuzzyDate. 
     * @param when a FuzzyDate
     * @return  true if and only if the instant represented by this Date object is strictly later than the instant represented by when; false otherwise. 
     */
    public boolean after(FuzzyDate when) {
        return getMaxDate().after(when.getMaxDate());
    }
    
    /**
     * Tests if this FuzzyDate is before the specified FuzzyDate. 
     * @param when a FuzzyDate
     * @return  true if and only if the instant represented by this Date object is strictly earlier than the instant represented by when; false otherwise. 
     */
    public boolean before(FuzzyDate when) {
        return getMinDate().before(when.getMinDate());
    }
    ...
    /**
     * Tests if this FuzzyDate is after the specified Date. 
     * @param when a FuzzyDate
     * @return  true if and only if the instant represented by this Date object is strictly later than the instant represented by when; false otherwise. 
     */
    ...
    /**
     * Tests if this FuzzyDate is before the specified Date. 
     * @param when a FuzzyDate
     * @return  true if and only if the instant represented by this Date object is strictly earlier than the instant represented by when; false otherwise. 
     */
    ...
    private void validateFormat(String dateString) throws ParseException {
        String[] dateParts= dateString.split("-");
        try {
        Integer.parseInt(dateParts[0]);
        if (Integer.parseInt(dateParts[0]) > 9999) {
            throw new ParseException("Year part of date string can not be more than 9999, was " + dateParts[0], 0);
        }
        if (dateParts.length >= 2 && Integer.parseInt(dateParts[1]) > 12) {
            throw new ParseException("Month part of date string can not be more than 12, was " + dateParts[1], 1);
        }
        if (dateParts.length >= 3 && Integer.parseInt(dateParts[2]) > 31) {
            throw new ParseException("Month part of date string can not be more than 31, was " + dateParts[2], 2);
        }
        } catch (Throwable t) {
            throw new ParseException("Invalid date format " + dateString +
                    ", the date must be of the format yyyy-MM-dd (-MM or -MM-dd are optional)", 0);
        }
    }
